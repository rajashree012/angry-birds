					...
					if (args.length == 1 && command.equalsIgnoreCase("-na"))
					{
						...
						if (args.length == 1 && command.equalsIgnoreCase("-nasc"))
						{
							...
							if (args.length == 2 && command.equalsIgnoreCase("-nasc"))
							{
								...
								if (args.length == 2 && command.equalsIgnoreCase("-nat"))
								{
									  NaiveAgentTest nat = new  NaiveAgentTest();
									  if(! args[1].equalsIgnoreCase("-showSeg"))
									  {
											int initialLevel = 1;
											try{
										  ...
										  nat.currentLevel = initialLevel;
							              nat.run();
									  }
									  else
									  {
										  Thread nathre = new Thread(nat);
										   nathre.start();
										   Thread thre = new Thread(new TestVision());
									      thre.start();
									  }
								...
								if (args.length == 2 && command.equalsIgnoreCase("-naf"))
								{
									  NaiveAgentFinal naf = new  NaiveAgentFinal();
									  if(! args[1].equalsIgnoreCase("-showSeg"))
									  {
											int initialLevel = 1;
											try{
										  ...
										  naf.currentLevel = initialLevel;
							              naf.run();
									  }
									  else
									  {
										  Thread nathre = new Thread(naf);
										   nathre.start();
										   Thread thre = new Thread(new TestVision());
									      thre.start();
									  }
